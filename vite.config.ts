  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (cleanPath === 'tmdb') {
          const action = urlObj.searchParams.get('action') || 'search';
          ...
          let passedKey = urlObj.searchParams.get('apiKey') || '';
          if (!passedKey && req.method === 'POST') {
            const body = await parseBody();
            if (body.apiKey) passedKey = body.apiKey;
          }
        ...
        if (cleanPath === 'download-zip' || pathname === '/api/download-zip' || pathname === '/htdocs.zip') {
          const zipPath = path.resolve(process.cwd(), 'public', 'htdocs.zip');
          const distZip = path.resolve(process.cwd(), 'dist', 'htdocs.zip');
          const target = fs.existsSync(zipPath) ? zipPath : fs.existsSync(distZip) ? distZip : null;
          if (target) {
