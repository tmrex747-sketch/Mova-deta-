...
import { Search, X, Film, Star, Check, Sparkles, Loader2, Tv } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl bg-[#0e1320] border border-white/10 shadow-2xl shadow-black/80 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                মুভি বা ওয়েব সিরিজের নাম লিখে সার্চ করুন — স্বয়ংক্রিয় ১৬:৯ থাম্বনেইল, টাইটেল ও তথ্য ফর্মটিতে ইমপোর্ট হবে।
              </p>
            </div>
          </div>
          <button
            ...
            <div className="relative flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="মুভি বা সিরিজের নাম (যেমন: Mirzapur, Panchayat, Jawan), TMDB/IMDb লিংক..."
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-400">
              <p className="text-sm">কোনো মুভি বা ওয়েব সিরিজ পাওয়া যায়নি। অন্য কোনো নাম দিয়ে সার্চ করুন।</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="relative aspect-video w-full rounded-lg overflow-hidden bg-slate-950 border border-white/5 mb-2.5">
                      ...
                      <div className="absolute top-2 left-2 flex items-center gap-1">
                        {isTv ? (
                          <span className="px-1.5 py-0.5 rounded-md bg-purple-600/90 text-[10px] font-bold text-white border border-purple-400/40 flex items-center gap-1 shadow-sm">
                            <Tv className="w-3 h-3" />
                            <span>Series</span>
                          </span>
                        ) : (
                          <span className="px-1.5 py-0.5 rounded-md bg-blue-600/90 text-[10px] font-bold text-white border border-blue-400/40 flex items-center gap-1 shadow-sm">
                            <Film className="w-3 h-3" />
                            <span>Movie</span>
                          </span>
                        )}
                        <span className="px-2 py-0.5 rounded-md bg-black/70 backdrop-blur-md text-[10px] font-semibold text-amber-300 border border-amber-500/30 flex items-center gap-1">
                      ...
                      <button
                        onClick={() => {
                          const cleanGenres = movie.genres && movie.genres.length > 0 
                            ? [...movie.genres] 
                            : (isTv ? ['#WebSeries', '#Drama'] : ['#Action', '#Thriller']);
                          if (isTv && !cleanGenres.includes('#WebSeries')) {
                          ...
                          onSelectMovie({
                            ...
                            genres: cleanGenres,
                            backdropUrl: img,
                            language: isTv ? 'Hindi / Dual Audio' : undefined,
                            id: movie.id,
                            backdrops: movie.backdrops
                          });
                          onClose();
                        }}
                        className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-amber-500/15 hover:bg-amber-500 text-amber-300 hover:text-black font-semibold text-xs border border-amber-500/30 transition active:scale-95 cursor-pointer"
                      >
        ...
        <div className="px-5 py-3 border-t border-white/10 bg-slate-900/60 flex items-center justify-between text-xs text-slate-400">
          <span>Click "Import" to load movie or web series info instantly</span>
          <button
            onClick={onClose}
